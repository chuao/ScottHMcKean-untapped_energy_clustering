def plot_top_n(well_data_df, top_k_index = pd.Int64Index([1]),  
    ...
    ax.set_title(f'Scatter Plot of Top {k} Wells')
    
# make a kNN predict function
def predict_print_knn(well_data_df, dist_func, k , target_index, target_column, title_str):
...
def kmeans_elbowplot(df, kmax = 12):
    ...
    for i in range(2, kmax):
        ...
        ch_score = metrics.calinski_harabasz_score(df, kmeans.labels_)
...
#     print(inertia)
    ch_score = 'Calinski Harabaz: ' + str(metrics.calinski_harabasz_score(df, cluster_res.labels_))
    print(ch_score)
